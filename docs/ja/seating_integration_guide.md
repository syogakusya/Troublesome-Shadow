...
2. PoseCaptureApp を `--seating-config` オプション付きで起動します。
   ...
   ```
3. 実行中は各フレームに `Meta.seating` フィールドが追加され、Unity から占有情報を受け取れるようになります。必要に応じて `--debug`（または GUI ランチャーの「デバッグログを有効化」）でログを詳細表示し、メタデータが送信されているか確認してください。【F:pose_capture/pose_capture_app.py†L57-L85】【F:pose_capture/seating.py†L32-L115】
...
2. カメラの映像内で椅子に着席・接近すると、`ShadowSeatDirector` が以下の挙動を行います。
   - **同じ椅子に座られた場合**: 影が驚いたリアクションを再生し、最も遠い空席へ移動します。空席が無ければ床に退避します。
   - **隣の椅子に座られた場合**: 影が迷惑そうに一席空けるよう離れた空席を探して移動します。空席が無ければ床に退避します。
   - **全席が人で埋まった場合**: 床アンカーへ移動し、困り果てたアニメーションを再生します。
   - **近くに人がいるだけの場合**: 信頼度が設定値を超え、クールダウンが明けていれば睨みつけアニメーションを再生し、視線を観客の方向に向けます。
   - **観客が離れた場合**: 影はデフォルト座席に戻るか、空いている席の中で優先順位が高い場所に座り直します。
