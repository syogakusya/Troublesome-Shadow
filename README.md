...
- CLI オプションを迷わずに起動したい場合は `python -m pose_capture.gui.launcher` を実行し、GUI でカメラインデックスやメタデータファイルを指定して PoseCaptureApp を開始できます。「デバッグログを有効化」をオンにすると `--debug` と同じく詳細ログが流れます。【F:pose_capture/gui/launcher.py†L1-L230】
...
- `--debug`: ルートロガーを DEBUG レベルに切り替え、`Meta.seating` などのメタデータをターミナルで確認できます。GUI ランチャーでは「デバッグログを有効化」をオンにすると同じ効果があります。
